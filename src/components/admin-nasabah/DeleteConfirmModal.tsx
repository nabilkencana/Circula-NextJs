    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl p-6 sm:p-7 shadow-2xl border border-gray-200 z-10 animate-in zoom-in-95 duration-200">
        ...
        <p className="text-xs sm:text-sm text-text-secondary mt-2 leading-relaxed">
          ...
          <span className="font-bold text-text-primary">{record.namaLengkap}</span> (
          <span className="font-mono font-semibold">{record.id}</span>)? Data buku induk dan
          riwayat transaksi terkait akan dihapus dari unit Anda.
        </p>
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          ...
          >
            {isSubmitting ? "Menghapus..." : "Ya, Hapus Nasabah"}
          </button>
        </div>
      </div>
    </div>
